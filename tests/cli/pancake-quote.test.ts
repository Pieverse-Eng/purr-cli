...
const abi = parseAbi(['function getAmountsOut(uint256 amountIn, address[] path) view returns (uint256[] amounts)'])

async function withRpc(run: (url: string, calls: any[]) => Promise<void>, fail = false) {
  const calls: any[] = []
  const server = createServer(async (req, res) => {
    ...
    calls.push(call)
    const result = call.method === 'eth_chainId' ? '0x38' : call.method === 'eth_blockNumber' ? '0x123' :
      encodeAbiParameters([{ type: 'uint256[]' }], [[BigInt(base.amountInWei), 123456789012345678901n]])
    res.setHeader('content-type', 'application/json')
    res.end(JSON.stringify(fail && call.method === 'eth_call' ?
      { jsonrpc: '2.0', id: call.id, error: { code: -32000, message: 'execution reverted' } } :
      { jsonrpc: '2.0', id: call.id, result }))
  })
  await new Promise<void>((resolve) => server.listen(0, '127.0.0.1', resolve))
  try { await run(`http://127.0.0.1:${(server.address() as any).port}`, calls) }
...
describe('Pancake V2 quote', () => {
  ...
  it('quotes at one block and floors minimum with bigint precision', async () => {
    await withRpc(async (rpcUrl, calls) => {
      ...
      expect(decoded.args[1].map((s) => s.toLowerCase())).toEqual(base.path.map((s) => s.toLowerCase()))
      expect(calls.every((c) => ['eth_chainId', 'eth_blockNumber', 'eth_call'].includes(c.method))).toBe(true)
    })
  })
  it('normalizes native BNB to WBNB', async () => {
    await withRpc(async (rpcUrl) => {
      const q = await quotePancakeSwap({ ...base, path: ['0x0000000000000000000000000000000000000000', B], rpcUrl, slippageBps: 0 })
      expect(q.path[0].toLowerCase()).toBe('0xbb4cdb9cbd36b01bd1cbaebf2de08d9173bc095c')
      expect(q.amountOutMinWei).toBe(q.amountOutWei)
    })
  })
  it.each([{ chainId: 1 }, { amountInWei: '0' }, { amountInWei: '-1' }, { slippageBps: 10000 }, { slippageBps: 0.5 }, { path: [A] }, { path: [A, ''] }])('rejects invalid input %j', async (override) => {
  ...
  it('CLI prints quote JSON without wallet credentials', async () => {
    await withRpc(async (rpcUrl) => {
      const result = await new Promise<{ code: number | null, stdout: string, stderr: string }>((resolve) => {
        const child = spawn('bun', ['packages/cli/src/linux-macos.ts', 'pancake', 'quote', '--path', `${A},${B}`, '--amount-in-wei', base.amountInWei, '--chain-id', '56', '--rpc-url', rpcUrl])
        let stdout = '', stderr = ''
