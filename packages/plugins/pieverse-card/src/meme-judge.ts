...
export async function pieverseMemeJudge(
  command: string | undefined,
  args: Record<string, string>,
): Promise<void> {
  if (args['rpc-url']) {
    throw new Error(
      'purr pieverse meme-judge commands do not accept --rpc-url. Set EVM_RPC_56 or EVM_RPC_URL if an RPC override is needed.',
    )
  }
  for (const name of CARD_ONLY_ARGS) {
    if (args[name] !== undefined) {
      throw new Error(
        `purr pieverse meme-judge commands do not accept --${name}; campaign channel and partner are selected by the platform.`,
      )
    }
  }
  ...
  switch (command) {
    ...
    default:
      throw new Error(
        'Unknown pieverse meme-judge command. Use: purchase, create-job, fund, result, status, input, refund',
      )
  }
