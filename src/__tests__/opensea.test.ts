...
function _makeBasicListing(
  overrides?: Partial<OpenSeaBestListingResponse>,
): OpenSeaBestListingResponse {
  return {
...
function _makeBasicOffer(overrides?: Partial<OpenSeaBestOfferResponse>): OpenSeaBestOfferResponse {
  return {
    order_hash: ORDER_HASH,
    chain: 'ethereum',
    protocol_address: OPENSEA_SEAPORT_V1_6,
      ...
      if (typeof url === 'string' && url.includes('/wallet/execute')) {
        ...
        return {
          ...
          json: () =>
            Promise.resolve({
              results: [
                { stepIndex: 0, label: body.steps?.[0]?.label, hash: '0xabc', status: 'success' },
              ],
              from: WALLET,
              chainId: 1,
              chainType: 'evm',
      ...
      if (typeof url === 'string' && url.includes('/wallet/execute')) {
        ...
        return {
          ...
          json: () =>
            Promise.resolve({
              results: [
                { stepIndex: 0, label: body.steps?.[0]?.label, hash: '0xdef', status: 'success' },
              ],
              from: WALLET,
              chainId: 1,
              chainType: 'evm',
    ...
    const mock = vi.fn().mockImplementation(async (url: string, init?: RequestInit) => {
      ...
      if (typeof url === 'string' && url.includes('/nfts/')) {
        return {
          ok: true,
          status: 200,
          json: () => Promise.resolve(nft),
          text: () => Promise.resolve(JSON.stringify(nft)),
        }
      }
      if (typeof url === 'string' && url.includes('/wallet/ensure')) {
        return {
