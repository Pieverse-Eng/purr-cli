...
Trading commands:
  ...
  algo cancel --algo-order-id <id> [--execute true]
...
function asRows(value: unknown): JsonRecord[] {
  if (Array.isArray(value)) return value.filter((row): row is JsonRecord => typeof row === 'object' && row !== null && !Array.isArray(row))
  if (typeof value === 'object' && value !== null && 'rows' in value && Array.isArray(value.rows)) {
    return value.rows.filter((row): row is JsonRecord => typeof row === 'object' && row !== null && !Array.isArray(row))
  }
  return []
}
...
async function platformWallet(chainType: 'ethereum' | 'solana'): Promise<string> {
  ...
  const response = await apiGet<WalletResponse>(
    `/v1/instances/${encodeURIComponent(instanceId)}/wallet?chainType=${chainType}`,
  )
...
async function ensureSolanaWallet(): Promise<string> {
  try {
    ...
    const { instanceId } = resolveCredentials()
    const response = await apiPost<WalletResponse>(`/v1/instances/${encodeURIComponent(instanceId)}/wallet/ensure`, {
      chainType: 'solana',
    })
    const address = response.data?.address
    if (!response.ok || !address) throw new OrderlyCliError(response.error ?? 'Failed to create Solana managed wallet')
    return address
  }
}
...
async function signRawSolana(message: string): Promise<string> {
  const { instanceId } = resolveCredentials()
  const response = await apiPost<WalletResponse>(`/v1/instances/${encodeURIComponent(instanceId)}/wallet/sign`, {
    message,
    chainType: 'solana',
    scheme: 'raw',
  })
  if (!response.ok || !response.data?.signature) throw new OrderlyCliError(response.error ?? 'Solana raw signing failed')
...
async function signTypedData(
  ...
  if (!response.ok || !response.data?.signature) throw new OrderlyCliError(response.error ?? 'EIP-712 signing failed')
  return response.data.signature
}

async function lookupAccount(evmAddress: string, requiredAccount = true): Promise<string | undefined> {
  try {
    ...
    if (!requiredAccount && error instanceof OrderlyCliError && error.status === 404) return undefined
    throw error
  }
}
...
async function identity(): Promise<Identity> {
  ...
  if (!accountId) throw new OrderlyCliError('Orderly account is not registered. Run purr orderly onboard --chain-id <id> --execute true.')
...
async function privateRequest<T = unknown>(
  ...
  if (!response.ok || envelope.success === false || envelope.ok === false) {
    const data = typeof parsed === 'object' && parsed !== null ? parsed as JsonRecord : undefined
    throw new OrderlyCliError(
      envelope.message ?? envelope.error ?? asString(data?.message) ?? `Orderly ${method} ${path} failed`,
...
async function tokenMetadata(token: string, chainId: number): Promise<{ address: string; decimals: number }> {
  const response = await orderlyRequest<unknown>('GET', '/v1/public/token')
  const row = asRows(response).find((item) => asString(item.token)?.toUpperCase() === token.toUpperCase())
...
async function chainMetadata(chainId: number): Promise<JsonRecord> {
  const response = await orderlyRequest<unknown>('GET', query('/v1/public/chain_info', { broker_id: brokerId() }))
...
async function queryDepositFee(
	...
	vault: string,
	evmAddress: string,
	input: { accountId: `0x${string}`; brokerHash: `0x${string}`; tokenHash: `0x${string}`; tokenAmount: bigint },
	...
	if (!response.ok) throw new OrderlyCliError(`Deposit-fee RPC request failed for chain ${String(vault)}`)
	const json = record(await response.json())
	if (json.error) throw new OrderlyCliError('Orderly vault rejected the deposit-fee quote', { data: json.error })
	const result = asString(json.result)
	if (!result) throw new OrderlyCliError('Deposit-fee RPC response has no result')
	return decodeFunctionResult({ abi: VAULT_ABI, functionName: 'getDepositFee', data: result as `0x${string}` }).toString()
}
...
function decimalIsMultiple(value: string, tick: unknown): boolean {
  ...
  const [whole, fraction = ''] = normalizedTick.split('.')
...
async function validateOrder(order: JsonRecord): Promise<void> {
  ...
  if (!decimalIsMultiple(quantity, info.base_tick)) {
    throw new OrderlyCliError(`--quantity must be a multiple of base_tick ${String(info.base_tick)}`)
  }
  if (order.order_price !== undefined && !decimalIsMultiple(String(order.order_price), info.quote_tick)) {
  ...
  if (order.order_price !== undefined && Number(quantity) * Number(order.order_price) < Number(info.min_notional)) {
...
async function status(): Promise<void> {
  const result: JsonRecord = { publicReady: false, brokerConfigured: Boolean(process.env.ORDERLY_BROKER_ID), accountReady: false, keyReady: false, authReady: false, tradeReady: false, reasons: [] as string[] }
  try {
    await publicInfo('PERP_BTC_USDC')
    result.publicReady = true
  } catch (error) {
    ;(result.reasons as string[]).push(error instanceof Error ? error.message : 'Orderly public API unavailable')
  }
  if (!result.brokerConfigured) (result.reasons as string[]).push('ORDERLY_BROKER_ID is not configured')
  try {
    ...
    ;(result.reasons as string[]).push(error instanceof Error ? error.message : 'Managed wallet authentication unavailable')
  }
  result.tradeReady = result.publicReady === true && result.brokerConfigured === true && result.accountReady === true && result.keyReady === true && result.authReady === true
  print(result)
}
...
async function onboard(args: Record<string, string>): Promise<void> {
  ...
  const now = Date.now()
  const registrationNonce = existing ? undefined : await orderlyRequest<unknown>('GET', '/v1/registration_nonce')
  const nonceValue = typeof registrationNonce === 'object' && registrationNonce !== null && 'registration_nonce' in registrationNonce
    ? (registrationNonce as JsonRecord).registration_nonce
    : registrationNonce
  const registrationMessage = { brokerId: broker, chainId, timestamp: now, ...(nonceValue !== undefined ? { registrationNonce: String(nonceValue) } : {}) }
  const addKeyMessage = { brokerId: broker, chainId, orderlyKey, scope: 'read,trading,asset', timestamp: now, expiration: now + 31_536_000_000 }
  const preview = { execute: false, evmAddress, solanaAddress, existingAccountId: existing ?? null, orderlyKey, registration: existing ? { required: false } : { required: true, message: registrationMessage }, addOrderlyKey: { required: true, message: addKeyMessage } }
  if (!execute(args)) return print(preview)

  const domain = { name: 'Orderly', version: '1', chainId, verifyingContract: OFFCHAIN_VERIFYING_CONTRACT }
  let accountId = existing
  if (!accountId) {
    const signature = await signTypedData(domain, { Registration: [{ name: 'brokerId', type: 'string' }, { name: 'chainId', type: 'uint256' }, { name: 'timestamp', type: 'uint64' }, { name: 'registrationNonce', type: 'uint256' }] }, 'Registration', registrationMessage)
    const response = await orderlyRequest<JsonRecord>('POST', '/v1/register_account', { message: registrationMessage, signature, userAddress: evmAddress })
    accountId = asString(response.account_id) ?? asString(record(response.data ?? {}).account_id)
  }
  if (!accountId) throw new OrderlyCliError('Orderly did not return an account ID after registration')
  const keySignature = await signTypedData(domain, { AddOrderlyKey: [{ name: 'brokerId', type: 'string' }, { name: 'chainId', type: 'uint256' }, { name: 'orderlyKey', type: 'string' }, { name: 'scope', type: 'string' }, { name: 'timestamp', type: 'uint64' }, { name: 'expiration', type: 'uint64' }] }, 'AddOrderlyKey', addKeyMessage)
  await orderlyRequest('POST', '/v1/orderly_key', { message: addKeyMessage, signature: keySignature, userAddress: evmAddress })
...
async function deposit(args: Record<string, string>): Promise<void> {
  ...
  if (!vault || !isAddress(vault)) throw new OrderlyCliError(`Orderly chain ${chainId} has no executable EVM vault`)
	const tokenInfo = await tokenMetadata(token, chainId)
	const amountWei = parseUnits(amount, tokenInfo.decimals)
	const input = { accountId: identityContext.accountId as `0x${string}`, brokerHash: keccak256(stringToHex(brokerId())), tokenHash: keccak256(stringToHex(token)), tokenAmount: amountWei }
	const quotedFeeWei = await queryDepositFee(chainRpcUrl(chain, chainId), vault, identityContext.evmAddress, input)
	const feeWei = args['fee-wei'] ?? quotedFeeWei
	if (!feeWei) {
		if (execute(args)) throw new OrderlyCliError(`No RPC URL is available to quote the deposit fee for chain ${chainId}. Set ORDERLY_RPC_URL_${chainId} or supply the verified --fee-wei quote.`)
		return print({ execute: false, chainId, token, amount, amountWei: amountWei.toString(), steps: [], requires: `Set ORDERLY_RPC_URL_${chainId} or supply --fee-wei from a verified Vault getDepositFee quote.` })
	}
	if (!/^\d+$/.test(feeWei)) throw new OrderlyCliError('--fee-wei must be an integer wei amount')
  const steps = [
    { kind: 'approve', to: tokenInfo.address, signature: 'approve(address,uint256)', args: [vault, amountWei.toString()], chainId },
    { kind: 'deposit', to: vault, signature: 'deposit((bytes32,bytes32,bytes32,uint256))', args: [input.accountId, input.brokerHash, input.tokenHash, amountWei.toString()], value: feeWei, chainId },
  ]
	if (!execute(args)) return print({ execute: false, chainId, token, amount, amountWei: amountWei.toString(), feeWei, feeSource: args['fee-wei'] ? 'explicit' : 'vault_rpc', steps })
  const { instanceId } = resolveCredentials()
  const approval = await apiPost<WalletResponse>(`/v1/instances/${encodeURIComponent(instanceId)}/wallet/execute`, { to: tokenInfo.address, abi: ['function approve(address spender, uint256 amount) returns (bool)'], functionName: 'approve', args: [vault, amountWei.toString()], chainId })
  if (!approval.ok || !approval.data?.hash) throw new OrderlyCliError(approval.error ?? 'Orderly token approval failed')
  const deposited = await apiPost<WalletResponse>(`/v1/instances/${encodeURIComponent(instanceId)}/wallet/execute`, { to: vault, abi: ['function deposit((bytes32 accountId, bytes32 brokerHash, bytes32 tokenHash, uint256 tokenAmount) input) payable'], functionName: 'deposit', args: [input], value: feeWei, chainId })
  if (!deposited.ok || !deposited.data?.hash) throw new OrderlyCliError(deposited.error ?? 'Orderly deposit transaction failed')
  print({ execute: true, approveTxHash: approval.data.hash, depositTxHash: deposited.data.hash, accountId: identityContext.accountId })
}
...
async function withdraw(args: Record<string, string>): Promise<void> {
  ...
  const message = { brokerId: brokerId(), chainId, receiver, token, amount: parseUnits(amount, tokenInfo.decimals).toString(), withdrawNonce, timestamp: Date.now(), ...(args['allow-cross-chain'] === 'true' ? { allowCrossChainWithdraw: true } : {}) }
  const domain = { name: 'Orderly', version: '1', chainId, verifyingContract: API_URL.includes('testnet') ? TESTNET_LEDGER : MAINNET_LEDGER }
  if (!execute(args)) return print({ execute: false, message, domain, note: 'The withdrawal nonce is short lived; execute promptly and never retry a timed-out withdrawal without checking asset-history.' })
  const signature = await signTypedData(domain, { Withdraw: [{ name: 'brokerId', type: 'string' }, { name: 'chainId', type: 'uint256' }, { name: 'receiver', type: 'address' }, { name: 'token', type: 'string' }, { name: 'amount', type: 'uint256' }, { name: 'withdrawNonce', type: 'uint64' }, { name: 'timestamp', type: 'uint64' }] }, 'Withdraw', message)
...
function orderFromArgs(args: Record<string, string>): JsonRecord {
  ...
  if (!(ORDER_TYPES as readonly string[]).includes(type)) throw new OrderlyCliError(`Unsupported --type ${type}`)
  const order: JsonRecord = { symbol: required(args, 'symbol'), side: required(args, 'side').toUpperCase(), order_type: type, order_quantity: positiveDecimal(required(args, 'quantity'), 'quantity') }
  if (order.side !== 'BUY' && order.side !== 'SELL') throw new OrderlyCliError('--side must be BUY or SELL')
  if (type !== 'MARKET' && type !== 'ASK' && type !== 'BID') order.order_price = positiveDecimal(required(args, 'price'), 'price')
...
async function createOrder(args: Record<string, string>): Promise<void> {
  ...
  if (!execute(args)) return print({ execute: false, order, note: 'Reuse the same --client-order-id when executing this preview.' })
  print(await privateRequest('POST', '/v1/order', order))
}

async function updateOrder(args: Record<string, string>): Promise<void> {
  const body: JsonRecord = { order_id: required(args, 'order-id'), order_quantity: positiveDecimal(required(args, 'quantity'), 'quantity') }
...
async function cancelOrder(args: Record<string, string>): Promise<void> {
  const path = query('/v1/order', { order_id: required(args, 'order-id'), symbol: required(args, 'symbol') })
...
async function closePosition(args: Record<string, string>): Promise<void> {
  ...
  const context = await identity()
  const position = await privateRequest<unknown>('GET', `/v1/position/${encodeURIComponent(symbol)}`, undefined, context)
  const row = record(position)
  const qty = Number(row.position_qty)
  if (!Number.isFinite(qty) || qty === 0) throw new OrderlyCliError(`No open position for ${symbol}`)
  const percentage = args.percentage === undefined ? 100 : Number(args.percentage)
  if (!Number.isFinite(percentage) || percentage <= 0 || percentage > 100) throw new OrderlyCliError('--percentage must be between 1 and 100')
  const order = { symbol, side: qty > 0 ? 'SELL' : 'BUY', order_type: 'MARKET', order_quantity: (Math.abs(qty) * percentage / 100).toString(), reduce_only: true }
...
async function createAlgo(args: Record<string, string>): Promise<void> {
  ...
  if (args['take-profit']) children.push({ symbol, algo_type: 'TAKE_PROFIT', side, type: 'MARKET', trigger_price: positiveDecimal(args['take-profit'], 'take-profit'), reduce_only: true })
  if (args['stop-loss']) children.push({ symbol, algo_type: 'STOP_LOSS', side, type: 'MARKET', trigger_price: positiveDecimal(args['stop-loss'], 'stop-loss'), reduce_only: true })
  if (children.length === 0) throw new OrderlyCliError('Specify --take-profit and/or --stop-loss')
  const body = { symbol, algo_type: children.length === 2 ? 'TP_SL' : children[0].algo_type, quantity, trigger_price_type: 'MARK_PRICE', child_orders: children }
...
async function publicQuery(type: string, args: Record<string, string>): Promise<void> {
  const body: JsonRecord = { type, ...(args.symbol ? { symbol: args.symbol } : {}), ...(args.interval ? { interval: args.interval } : {}), ...(args['start-t'] ? { start_t: Number(args['start-t']) } : {}), ...(args['end-t'] ? { end_t: Number(args['end-t']) } : {}), ...(args.depth ? { max_depth: Number(args.depth) } : {}) }
...
export async function orderlyCommand(command: string, args: Record<string, string>): Promise<void> {
  switch (command) {
    ...
    case 'markets': {
      ...
      return print(queryText ? rows.filter((row) => JSON.stringify(row).toUpperCase().includes(queryText)) : rows)
    }
    case 'market': return print(await publicInfo(required(args, 'symbol')))
    case 'orderbook': return await publicQuery('orderbook', args)
    case 'candles': return await publicQuery('candles', args)
    case 'funding': return await publicQuery('fundingRateHistory', args)
    case 'networks': return print(await orderlyRequest('GET', query('/v1/public/chain_info', { broker_id: brokerId() })))
    case 'tokens': return print(await orderlyRequest('GET', '/v1/public/token'))
    case 'onboard': return await onboard(args)
    case 'account': return print(await identity())
    case 'balance': return print(await privateRequest('GET', '/v1/client/holding'))
    case 'positions': return print(await privateRequest('GET', args.symbol ? `/v1/position/${encodeURIComponent(args.symbol)}` : '/v1/positions'))
    case 'orders': return print(await privateRequest('GET', query('/v1/orders', { status: args.status, symbol: args.symbol, page: args.page, size: args.size })))
    case 'fills': return print(await privateRequest('GET', query('/v1/trades', { symbol: args.symbol, page: args.page, size: args.size })))
    case 'asset-history': return print(await privateRequest('GET', query('/v1/asset/history', { token: args.token, side: args.side, page: args.page, size: args.size })))
    case 'deposit': return await deposit(args)
    case 'withdraw': return await withdraw(args)
    case 'order-create': return await createOrder(args)
    case 'order-update': return await updateOrder(args)
    case 'order-cancel': return await cancelOrder(args)
    case 'orders-cancel-all': return await cancelAll(args)
    case 'position-close': return await closePosition(args)
    case 'leverage-get': return print(await privateRequest('GET', query('/v1/client/leverage', { symbol: required(args, 'symbol') })))
    case 'leverage-set': {
      const body = { symbol: required(args, 'symbol'), leverage: optionalInt(args, 'leverage') }
      if (body.leverage === undefined || body.leverage < 1) throw new OrderlyCliError('Missing or invalid --leverage')
      if (!execute(args)) return print({ execute: false, leverage: body })
      return print(await privateRequest('POST', '/v1/client/leverage', body))
    }
    case 'algo-create': return await createAlgo(args)
    case 'algo-list': return print(await privateRequest('GET', query('/v1/algo/orders', { symbol: args.symbol })))
    case 'algo-cancel': {
      const path = query('/v1/algo/order', { algo_order_id: required(args, 'algo-order-id') })
      if (!execute(args)) return print({ execute: false, method: 'DELETE', path })
      return print(await privateRequest('DELETE', path))
    }
    default: throw new OrderlyCliError(`Unknown Orderly command: ${command}.\n${ORDERLY_USAGE}`)
  }
}
