...
describe('resolve', () => {
  it('parseQualifiedSlug detects source prefix', async () => {
    const { parseQualifiedSlug } = await import('@pieverseio/purr-plugin-store/resolve')
  ...
  it('resolveSlug returns unique when qualified slug is explicit', async () => {
    const { resolveSlug, SOURCES } = await import('@pieverseio/purr-plugin-store/resolve')
  ...
  it('resolveSlug returns not_found for missing qualified slug', async () => {
    const { resolveSlug } = await import('@pieverseio/purr-plugin-store/resolve')
